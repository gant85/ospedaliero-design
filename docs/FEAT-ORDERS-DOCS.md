...
This functional flow covers the **ORDINI**, **FATTURE**, **DDT**, and **REPORT** macro-areas. It tracks the chronological progression logically from Cart Assembly to Order Fulfillment, followed by Document Retrieval (Invoices/Delivery Notes) and finally Analytical Reporting.
...
1. **Cart & Fulfillment Pathing**: The cart API allows the user front-end to explicitly denote the sourcing context (e.g. `source=magazzino_ospedaliero`). The BFF passes this flag to the underlying Order Engine ensuring the logistics network processes it correctly upon checkout.
