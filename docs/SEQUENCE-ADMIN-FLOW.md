...
2. **Context Switching (Impersonation Pattern)**:
   - Unlike standard users who see only their own facility's data, Administrators have a "Scelta Cliente" (Client Selection) step in their flows (`Fatture`, `DDT`, `Storico Ordini`, `Report`, etc.).
   - The BFF acts as an intermediary, requesting the external APIs to return data scoped to the selected `client_id` or `struttura_id`, overriding the user's default context but maintaining auditability.

3. **Administration Tasks**:
   - The "Amministrazione" menu unlocks features like user management, file imports (*Importa Ospedaliero*), and modifying news/offers (*Gestione Offerte*).
